...
__global__ void kernel_march_rays_train(
    ...
    while (t < far && num_steps < max_steps) {
        ...
        const int nx = clamp(0.5 * (x * mip_rbound + 1) * grid_size, 0.0f, (float)(grid_size - 1));
        const int ny = clamp(0.5 * (y * mip_rbound + 1) * grid_size, 0.0f, (float)(grid_size - 1));
        const int nz = clamp(0.5 * (z * mip_rbound + 1) * grid_size, 0.0f, (float)(grid_size - 1));
    ...
    if (point_index + num_steps > max_points) return;
...
void train_raymarching(const at::Tensor rays_o, const at::Tensor rays_d, const at::Tensor grid, const float scale, const float dt_gamma, const uint32_t max_steps, const uint32_t num_rays, const uint32_t num_cascades, const uint32_t grid_size, const uint32_t max_points, const at::Tensor nears, const at::Tensor fars, at::Tensor xyzs, at::Tensor dirs, at::Tensor deltas, at::Tensor rays, at::Tensor counter) {
    ...
    rays_o.scalar_type(), "march_rays_train", ([&] {
        kernel_march_rays_train<<<div_round_up(num_rays, N_THREAD), N_THREAD>>>(rays_o.data_ptr<scalar_t>(), rays_d.data_ptr<scalar_t>(), grid.data_ptr<uint8_t>(), scale, dt_gamma, max_steps, num_rays, num_cascades, grid_size, max_points, nears.data_ptr<scalar_t>(), fars.data_ptr<scalar_t>(), xyzs.data_ptr<scalar_t>(), dirs.data_ptr<scalar_t>(), deltas.data_ptr<scalar_t>(), rays.data_ptr<int>(), counter.data_ptr<int>());
    }));
}
