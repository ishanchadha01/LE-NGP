...
void ray_intersection(const at::Tensor rays_o, const at::Tensor rays_d, const at::Tensor aabb, const uint32_t num_rays, const float min_near, at::Tensor nears, at::Tensor fars) {
    ...
    rays_o.scalar_type(), "ray_intersection", ([&] {
        kernel_ray_intersection<<<div_round_up(num_rays, N_THREAD), N_THREAD>>>(rays_o.data_ptr<scalar_t>(), rays_d.data_ptr<scalar_t>(), aabb.data_ptr<scalar_t>(), num_rays, min_near, nears.data_ptr<scalar_t>(), fars.data_ptr<scalar_t>());
    }));
}