...
void train_composite_rays_forward(const at::Tensor sigmas, const at::Tensor rgbs, const at::Tensor deltas, const at::Tensor rays, const uint32_t max_points, const uint32_t num_rays, const float T_thresh, at::Tensor weights_sum, at::Tensor depth, at::Tensor image) {
    ...
    sigmas.scalar_type(), "train_composite_rays_forward", ([&] {
        kernel_train_composite_rays_forward<<<div_round_up(num_rays, N_THREAD), N_THREAD>>>(sigmas.data_ptr<scalar_t>(), rgbs.data_ptr<scalar_t>(), deltas.data_ptr<scalar_t>(), rays.data_ptr<int>(), max_points, num_rays, T_thresh, weights_sum.data_ptr<scalar_t>(), depth.data_ptr<scalar_t>(), image.data_ptr<scalar_t>());
    }));
}
