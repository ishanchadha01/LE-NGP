...
class BaseTrainer(object):
    ...
    def train(self, cfg, data_loader, single_gpu=False, profile=False, show_pbar=False, metrics=None):
        ...
        Args:
            ...
            metrics (dict): Metrics to show in progress bar
        """
        ...
        for current_epoch in range(start_epoch, cfg.max_epoch):
            ...
            if show_pbar:
                data_loader_wrapper = tqdm(data_loader, desc=f"Training epoch {current_epoch + 1} psnr:{metrics['psnr']}", leave=False)
            else:
